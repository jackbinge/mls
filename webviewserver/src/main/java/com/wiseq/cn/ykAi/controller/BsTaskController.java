...
public class BsTaskController {
     ...
     */
    @PutMapping("/setNG")
    @ApiOperation("设置NG--2.0（新加参数：judgementResult）[改参数]")
    ...
    Result cancelNG(@RequestParam(value = "taskStateId")Long taskStateId,
        ...
        return this.bsTaskService.cancelNG(taskStateId, eqptValveIdList, userId, raRequire, compulsoryPass, updateChipArea);
     ...
     */
    @PutMapping("/testOk")
    @ApiOperation("测试OK -- 2.0 废弃不对前端开放所有都走setNG接口[废弃了]")
    ...
    Result addFormulaEdit(@RequestBody EditTaskFormulaForPage editTaskFormulaForPage){
        return this.bsTaskService.addFormulaEdit(editTaskFormulaForPage);
    }
    ...
    @ApiOperation("推荐点胶量[改参数]")
    Result recommendGlueDosageMethod(@RequestParam("eqptValveId") Long eqptValveId,
    ...
    @ApiOperation("配比推荐数据源之模型对应的工单列表[废弃]")
    ...
    @ApiOperation("获取已经关闭的住流程工单[新增接口]")
