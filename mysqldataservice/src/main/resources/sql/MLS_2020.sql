...
-- 2020-05-21修改改动
   -- rar9Type 字段移除
   -- 新增 system_task_type 字段 0 EAS工单，1 是纯打样工单
   -- 新增 close_user 关闭人 关闭工单的用户
...
(
    ...
    system_task_type int   comment '0 EAS工单，1是纯打样工单',
    close_user  bigint comment  '关闭工单的用户',
...
-- 工单状态流转表
-- 2020-05-21修改改动（主流程的一些信息设计到EAS工单投产，因此EAS工单投产可能改动比较大）
    -- 去掉 reason, solution_type,modify_time,creator,update_user,check_user,is_retest
    -- 新增 model_create_time 配方创建时间/修改时间
    -- 新增 ratio_create_time 配比创建时间/修改时间
    -- 新增 process_type 流程类型 0 主流程，1 打样流程
    -- 新增 process_create_time 流程开始时间(主流程为EAS工单投产的时间，打样流程为发起打样的时间)
    -- 新增 process_version 流程版本（主流程默认为0不变，打样流程从零开始 每发起一次 版本号+1，从 1开始计算）
    -- 新增 process_initiator 流程的发起人 （主流程为EAS投产人，打样流程为打样发起人）
    -- 新增 initiate_reason 发起流程的原因 0：此为主流程，1：荧光粉批次变更，2：在制物料库存不足，3：评估新物料
    -- 新增 model_version 配方版本 每次修改后加1 从0开始
    -- 新增 ratio_version 配比版本 每次修改后加1 从0开始
    -- 新增 model_creator 配方操作人/创建人 主流程第一个配方是 投产用户，打样流程的第一个配方操作人时发起打样流程的人
    -- 新增 ratio_creator 配比操作人/创建人 主流成第一个配比是 投产用户，打样流程的第一个配比操作人是发起打样流程的人
    -- 新增 rar9_type    tinyint       default 1 comment '0 忽略 1不忽略'
    -- 新增 ratio_source 配比来源 0 配比数据库中，1 系统推荐，2 用户编辑, 3.打点推荐
...
(
   ...
   -- solution_type                      tinyint comment '解决措施 成功为0, 1忽略继续生产  2修改点胶量 3修改配比 4 修改bom 5 前测数据批量否定',
    create_time                        datetime not null default now() comment '创建时间/此条记录的生成时间',
   ...
   -- check_user                         bigint comment '状态变更修改确认用户',
    model_create_time                  datetime not null  comment '配方创建时间/修改时间',
    ratio_create_time                  datetime  comment '配比创建时间/修改时间',
    process_type                       int not null comment '流程类型 0 主流程，1 打样流程',
    process_create_time                datetime not null comment '流程开始时间(主流程为EAS工单投产的时间，打样流程为发起打样的时间)',
    process_version                    int not null comment '流程版本（主流程默认为0不变，打样流程从零开始 每发起一次 版本号+1，从 1开始计算）',
    process_initiator                  bigint not null comment ' 流程的发起人 （主流程为EAS投产人，打样流程为打样发起人）',
    initiate_reason                    varchar(200) not null comment '发起流程的原因 ',
    model_version                      int not null comment '配方版本 每次修改后加1 从0 开始',
    ratio_version                      int  comment '配比版本 每次修改后加1 从0 开始',
    model_creator                      bigint not null comment '配方操作人/创建人 主流程第一个配方是 投产用户，打样流程的第一个配方操作人时发起打样流程的人',
    ratio_creator                      bigint  comment '配比操作人/创建人 主流成第一个配比是 投产用户，打样流程的第一个配比操作人是发起打样流程的人',
    rar9_type                          tinyint       default 1 comment '0 忽略 1不忽略',
    ratio_source                       int           comment '配比来源 0 配比数据库中，1 系统推荐，2 用户编辑, 3打点推荐 没有配比时为null',
...
-- 分光结果
-- 2020-05-21修改改动
   -- 新增字段 judge_type 0 正常判定为 ok,1 NG 后改判,2 强制通过
...
(
    ...
    judge_type    int          comment '0 正常判定为 ok,1 NG 后改判,2 强制通过',
...
create table d_upload_file_judgement_result(
    ...
    lightness       int       default  null comment '亮度判定结果,0 OK，1 NG, 2 偏高, 3偏低',
    ra              int       default  null comment 'ra的判定结果,0 OK，1 NG, 2 偏高, 3偏低',
    r9              int       default  null comment 'r9的判定结果,0 OK，1 NG, 2 偏高, 3偏低'
...
(
    ...
    r9_yield      double   comment 'r9良率',
    ra_target     double   comment  '显色指数,目标值/ra系列',
    ra_re_max     double   comment '限制范围上限，用于算法显指良率统计',
    ra_re_min     double   comment '显色指数下限，用于算法显指良率统计',
    r9            double   comment 'R9',
    lumen_lsl     double   comment '流明下限',
    lumen_usl     double   comment '流明上限    ',
...
-- ----------------------------
-- Table structure for bs_z_consumption
-- ----------------------------
DROP TABLE IF EXISTS `bs_z_consumption`;
CREATE TABLE `bs_z_consumption` (
  `id` bigint(20) NOT NULL AUTO_INCREMENT,
  `task_id` bigint(20) NOT NULL COMMENT '任务单id',
  `task_state_id` bigint(20) NOT NULL COMMENT '任务单状态id',
  `glue_a_material` double DEFAULT NULL COMMENT 'a胶配粉重量',
  `glue_b_materiad` double DEFAULT NULL COMMENT 'b胶配粉重量',
  `anti_starch_material` double DEFAULT NULL COMMENT '抗沉淀粉配粉重量',
  `diffusion_powder_material` double DEFAULT NULL COMMENT '扩散粉配粉重量',
  `t_phosphors0` double DEFAULT NULL COMMENT '荧光粉1配粉重量',
  `t_phosphors1` double DEFAULT NULL COMMENT '荧光粉2配粉重量',
  `t_phosphors2` double DEFAULT NULL COMMENT '荧光粉3配粉重量',
  `t_phosphors3` double DEFAULT NULL COMMENT '荧光粉4配粉重量',
  `t_phosphors4` double DEFAULT NULL COMMENT '荧光粉5配粉重量',
  PRIMARY KEY (`id`)
) ENGINE=InnoDB AUTO_INCREMENT=4 DEFAULT CHARSET=utf8 COLLATE=utf8_bin COMMENT='保存配粉重量作为上次配粉重量';

-- ----------------------------
-- Table structure for t_algorithm_model
-- ----------------------------
DROP TABLE IF EXISTS `t_algorithm_model`;
CREATE TABLE `t_algorithm_model`  (
  `id` bigint(20) NOT NULL AUTO_INCREMENT,
  `scaffold_id` bigint(20) NULL DEFAULT NULL COMMENT '支架id',
  `scaffold_name` varchar(100) CHARACTER SET utf8 COLLATE utf8_general_ci NULL DEFAULT NULL COMMENT '支架名称',
  `scaffold_param` varchar(100) CHARACTER SET utf8 COLLATE utf8_general_ci NULL DEFAULT NULL COMMENT '支架参数',
  `scaffold_re` double(10, 6) NULL DEFAULT 0.000000 COMMENT '支架要求',
  `chip_id` bigint(20) NULL DEFAULT NULL COMMENT '芯片id',
  `chip_name` varchar(100) CHARACTER SET utf8 COLLATE utf8_general_ci NULL DEFAULT NULL COMMENT '芯片名称',
  `chip_wave` int(255) NULL DEFAULT NULL COMMENT '芯片波长',
  `ct` int(255) NULL DEFAULT NULL COMMENT '色温',
  `p1_id` bigint(20) NULL DEFAULT NULL COMMENT '粉1id',
  `p1_name` varchar(100) CHARACTER SET utf8 COLLATE utf8_general_ci NULL DEFAULT NULL COMMENT '粉1名称',
  `p1_wave` int(255) NULL DEFAULT NULL COMMENT '粉1波长',
  `p2_id` bigint(20) NULL DEFAULT NULL COMMENT '粉2id',
  `p2_name` varchar(100) CHARACTER SET utf8 COLLATE utf8_general_ci NULL DEFAULT NULL COMMENT '粉1名称',
  `p2_wave` int(255) NULL DEFAULT NULL COMMENT '粉2波长',
  `p3_id` bigint(20) NULL DEFAULT NULL COMMENT '粉3id',
  `p3_name` varchar(100) CHARACTER SET utf8 COLLATE utf8_general_ci NULL DEFAULT NULL COMMENT '粉3名称',
  `p3_wave` int(255) NULL DEFAULT NULL COMMENT '粉3波长',
  `a_ratio` double(255, 6) NULL DEFAULT NULL COMMENT 'A胶比例',
  `b_raito` double(255, 6) NULL DEFAULT NULL COMMENT 'B胶比例',
  `c_ratio` double(255, 6) NULL DEFAULT NULL COMMENT '沉淀粉比例',
  `p1_ratio` double(255, 6) NULL DEFAULT NULL COMMENT '粉1比例',
  `p2_ratio` double(255, 6) NULL DEFAULT NULL COMMENT '粉2比例',
  `p3_ratio` double(255, 6) NULL DEFAULT NULL COMMENT '粉3比例',
  `center_xy` varchar(100) CHARACTER SET utf8 COLLATE utf8_general_ci NULL DEFAULT NULL COMMENT '打靶中心',
  `ra_mean` double(255, 6) NULL DEFAULT NULL COMMENT 'ra均值',
  `ra_min` double(255, 6) NULL DEFAULT NULL COMMENT 'ra最小值',
  `ra_max` double(255, 6) NULL DEFAULT NULL COMMENT 'ra最大值',
  `r9_mean` double(255, 6) NULL DEFAULT NULL COMMENT 'r9均值',
  `r9_min` double(255, 6) NULL DEFAULT NULL COMMENT 'r9最小值',
  `r9_max` double(255, 6) NULL DEFAULT NULL COMMENT 'r9最大值',
  `lm_mean` double(255, 6) NULL DEFAULT NULL COMMENT '亮度均值',
  `lm_min` double(255, 6) NULL DEFAULT NULL COMMENT '亮度最小值',
  `lm_max` double(255, 6) NULL DEFAULT NULL COMMENT '亮度最大值',
  `file_name` varchar(255) CHARACTER SET utf8 COLLATE utf8_general_ci NULL DEFAULT NULL COMMENT '分光文件',
  PRIMARY KEY (`id`) USING BTREE
) ENGINE = InnoDB AUTO_INCREMENT = 40 CHARACTER SET = utf8 COLLATE = utf8_general_ci COMMENT = '算法模型表' ROW_FORMAT = Dynamic;
...
FROM
    bs_task
        ...
        JOIN d_upload_file ON (FIND_IN_SET(d_upload_file.id,bs_task_state.fileid_list))
...
WHERE
 ...
 -- AND bs_task_state.is_retest = FALSE
  AND d_file_summary.euclidean_distance_x <= 0.1
  AND d_file_summary.euclidean_distance_y <= 0.1 AND d_file_summary.total_size >= 50
